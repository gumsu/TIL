...
### 발생 조건
- 상호배제
  - 한 리소스는 한 번에 하나의 프로세스만이 사용할 수 있다.
- 점유와 대기
  - 하나 이상의 자원을 점유한 채, 다른 프로세스에 의해 점유된 자원을 추가로 얻기 위해 대기한다.
- 비선점
  - 자원을 강제로 빼앗을 수 없고 프로세스가 자발적으로 반환해야 한다.
- 순환대기
  - hold and wait 관계의 프로세스들이 서로 기다려야 한다.

### 해결 방법
- 예방
  - 필요 조건 중 하나가 성립하지 않도록 보장
- 회피
  - 프로세스가 요구할 자원에 대한 부가적인 정보를 미리 제공하도록 요구하여 발생 가능성이 있으면 지연시킨다.
- 무시
  - 교착상태가 발생할 수 있도록 두고, 회복 방법을 지원한다.

## 기아상태
- 특정 프로세스의 우선순위가 낮아서 자원을 계속 할당받지 못하는 상태

### 교착상태 vs 기아상태
- 교착상태는 동일한 리소스를 동시에 점유하기 위해 요청할 때 발생한다.
- 기아상태는 한정된 리소스를 점유하기 위해 경쟁할 때 발생한다.

### 해결 방법
- 오래 기다린 프로세스에게 점점 우선순위를 높여준다.
- 수시로 우선순위를 변경한다.
- 우선순위 방식이 아닌 요청이 온 순서대로 처리하는 큐를 사용한다.

## 가상 메모리
- 물리 메모리 크기의 한계를 극복하기 위해 나온 기술이다.
- 물리 메모리보다 큰 프로세스를 수행하기 위해 사용한다.
- CPU에 실행되려면 실행에 당장 필요한 부분이 메모리에 올라와 있어야 한다. 여러 프로그램이 동시에 수행되기 위해서는 메모리가 커져야 하는데, 현실적으로 한계가 있다.
- 가상 메모리는 현재 필요한 부분만 메모리에 적재하는 것이다.
- 프로세스의 주소 공간을 메모리에 적재하는 단위에 따라 가상메모리 기법은 `페이징`과 `세그멘테이션`으로 나눈다.

### 가상 메모리와 MMU
- CPU가 프로세스를 실행할 때는 가상 메모리 주소를 사용하고, 가상 주소에서 데이터를 읽고 쓸 때만 물리 메모리 주소로 접근해서 실행한다.
- MMU (Memory Management Unit)
  - 가상 주소와 물리 주소를 변환해주는 것
- 가상 주소
  - 프로세스가 참조하는 주소 (0~4GB)
- 물리 주소
  - 실제 메모리 주소 (0~4GB 중 일부만)
...
### 세그멘테이션
- 서로 다른 크기로 분할해서 메모리에 적재하는 방식
- 페이징과 같이 물리적 단위가 아닌 논리적 단위로 분할한다.
- 외부 단편화 발생

## 캐시
- 캐시는 CPU에 들어가 있는 작은 메모리이다.
- 매번 CPU가 메인 메모리에 접근해 데이터를 받아오면 오래 걸리므로 자주 사용되는 데이터를 캐시에 담아두고 처리 속도를 빠르게 한다.

### 메인메모리 = 주기억장치 = RAM
- CPU가 직접 접근하는 장치
- 프로그램이 실행되려면 프로그램을 복사해서 메모리에 적재해야 한다.

### 캐시의 종류
- L1, L2, L3 캐시가 있다.

### 지역성
- 기억 장치 내에 균일하게 데이터에 접근하는 것이 아닌 어느 한 순간에 특정 부분을 집중적으로 참조하는 경향
- 공간 지역성
  - 최근 접근한 데이터의 주변에 다시 접근하는 경향
- 시간 지역성
  - 최근 접근한 데이터에 다시 접근하는 경향

### 캐시 vs 레지스터
- 캐시는 CPU와 별도의 공간, 메인 메모리와 CPU 간의 속도 차를 극복하기 위한 것이다.
- 레지스터는 CPU 안의 연산을 처리하기 위한 데이터 공간이다.
